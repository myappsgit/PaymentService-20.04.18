...
public class HttpUtils {
	...
	public static String sendPostRequest(String url, Map<String, String> headers, Map<String, String> params)
		...
		if ((params != null) && (params.size() > 0)) {
			...
			for (Map.Entry<String, String> param : params.entrySet()) {
				nameValuePairs.add(new BasicNameValuePair((String) param.getKey(), (String) param.getValue()));
			}
	...
	private static void addHeadersToHttpRequest(HttpRequestBase httpRequestBase, Map<String, String> headers) {
		if ((headers != null) && (headers.size() > 0)) {
			for (Map.Entry<String, String> header : headers.entrySet()) {
				httpRequestBase.addHeader((String) header.getKey(), (String) header.getValue());
			}
		}
	}
	...
	private static String getHttpResponseAsString(HttpResponse httpResponse) {
		...
		try {
			Reader reader = new InputStreamReader(httpResponse.getEntity().getContent(), Charset.forName("UTF-8"));
		...
		return stringResponse.toString().replaceAll(" ", "");
	}
}
